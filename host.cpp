...
int main(int argc, char** argv) {
    ...
    int source_in2_2d[DIMENSIZE][DIMENSIZE];
    for (int i = 0; i < DIMENSIZE; i++) {
        for (int j = 0; j < DIMENSIZE; j++) {
            source_in2_2d[i][j] = source_in2[i * DIMENSIZE + j];
        }
    }

    int transposed[DIMENSIZE][DIMENSIZE];
    for (int i = 0; i < DIMENSIZE; i++) {
        for (int j = 0; j < DIMENSIZE; j++) {
            transposed[j][i] = source_in2_2d[i][j];
        }
    }

    for (int i = 0; i < DIMENSIZE; i++) {
        for (int j = 0; j < DIMENSIZE; j++) {
            source_in2[i * DIMENSIZE + j] = transposed[i][j];
        }
    }
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_mmult = cl::Kernel(program, "vadd", &err)); // Update kernel name
            valid_device = true;
            break;
        }
    }
    ...
    int size = DATA_SIZE;
    int nargs = 0;
    OCL_CHECK(err, err = krnl_mmult.setArg(nargs++, buffer_in1)); // Update kernel arguments
    OCL_CHECK(err, err = krnl_mmult.setArg(nargs++, buffer_in2));
    OCL_CHECK(err, err = krnl_mmult.setArg(nargs++, buffer_output));
    OCL_CHECK(err, err = krnl_mmult.setArg(nargs++, size));
    et.finish();
    ...
    OCL_CHECK(err, err = q.enqueueTask(krnl_mmult)); // Update kernel launch
    et.finish();
